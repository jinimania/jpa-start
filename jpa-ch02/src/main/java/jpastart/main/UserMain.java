...
public class UserMain {
  ...
  private static void handleWithdrawCommand(final String[] cmd) {
    if (cmd.length != 2) {
  ...
  private static void handleListCommand() {
    final List<User> users = listService.getAllUsers();
    if (users.isEmpty()) {
      System.out.println("사용자가 없습니다.");
    } else {
      users.forEach(
          user -> System.out.printf("| %s | %s | %tY-%<tm-%<td |\n", user.getEmail(),
              user.getName(), user.getCreateDate())
      );
    }
  }

  private static void handleChangeName(final String[] cmd) {
    if (cmd.length != 3) {
      System.out.println("명령어가 올바르지 않습니다.");
  ...
  private static void handleViewCommand(final String[] cmd) {
    if (cmd.length != 2) {
      System.out.println("명령어가 올바르지 않습니다.");
      System.out.println("사용법: view 이메일");
      return;
    }
    final Optional<User> userOpt = getUserService.getUser(cmd[1]);
  ...
  private static void handleJoinCommand(final String[] cmd) {
    if (cmd.length != 3) {
      System.out.println("명령어가 올바르지 않습니다.");
